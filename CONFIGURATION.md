...
| `GOVERN_AGENT_WALLCLOCK` | `3600` (1h) | The interactive lane's wall-clock ceiling (`templates/hooks/agent-watchdog-guard.sh`, a `PreToolUse` hook scoped to subagent sessions): past this many seconds since a child's first tool call, its next tool call is denied with instructions to stop and report. `0` disables it. Starting point, ported unchanged from the headless launcher's `GOVERN_WORKER_TIMEOUT` |
| `GOVERN_AGENT_TOKEN_BUDGET` | `10000000` | The interactive lane's cumulative token ceiling for one subagent (same hook). Past this many tokens (summed off the child's own transcript), its next tool call is denied the same way. `0` disables it. Starting point, not a derived constant — unlike the headless launcher's own `GOVERN_WORKER_MAX_TOKENS`, which ships off, this ships **on** (rail 7: an inert-by-default watchdog is the G7 defect). Neither knob is `GOVERN_AGENT_SUPERVISION` (D8's idle-supervision switch) — each mechanism keeps its own kill switch |
| `GOVERN_PROPOSAL_GATE` | `1` (on) | Refuses a worker dispatch on a ticket that carries no `**Proposed solution:**`, on both lanes: `templates/govern/pre-dispatch-check.sh` prints `refuse:` for the headless launcher, and `templates/hooks/router-posture-guard.sh` denies the in-session `Agent` call. Deciding what the change is belongs to the advisor session, so a worker handed a bare problem statement re-derives it at the wrong tier. `0` turns both off. The hook degrades to allow whenever it cannot resolve the ticket, so it refuses only a ticket it can read |
...
| `GOVERN_ADVISOR_PER_WORKER` | `2` | Consults one worker may claim for one ticket. `spawn-worker.sh` overrides it per dispatch via `GOVERN_ADVISOR_BUDGET`, derived from the ticket's precision grade (`open` 3, scoped 2, `stated` 1) |
