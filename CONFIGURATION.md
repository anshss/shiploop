...
| `GOVERN_BATCH_MAX` | `1` (off = singletons) | Locality batching: `govern::locality_groups` folds up to this many tickets sharing measured file paths into one named group, dispatched to one worker that opens one branch and one PR. The worker's report gains a `tickets` array, one entry per group member, shaped `{"ticket":N,"status":"resolved\|parked\|failed","note":"..."}`; `resolve-ticket.sh` lands each `resolved` entry and leaves every other entry (or a ticket the array omits) in the queue. `1` (the default) makes every candidate a singleton, unchanged from today |
