...
- `commands/*.md` — the slash commands (`setup`, `update`, `govern`, `flows`, `push`). Judgment only; mechanical work belongs in `scaffold.sh` or a template script.
...
**The inverse failure is silent, and it bit us.** A test asserting on the *hub itself* — `scaffold.sh`, `templates/`, `VERSION`, `purge.txt`, `seed-hashes.txt` — resolves `HUB="$DIR/../../.."` and exits `77` ("skip") when that is not a hub checkout. Inside `scaffold-and-test` that path is the throwaway workspace root, so the test always skips, and `77` is counted as a skip rather than a failure. Result: five update-channel tests (`test-update-channel`, `test-purge-removed`, `test-reconcile-update`, `test-scaffold-verify-standalone`, `test-update-config-converge`) ran green locally and **never executed in CI at all** — the entire mechanism behind `/shiploop:update` was ungated while looking gated. Hub-context tests belong in the `hub-context-tests` job, which runs them from the checkout and treats a `77` as a hard failure. If you add a test that guards on `not running from a hub checkout`, add it to that job's list too, or it silently does nothing.
