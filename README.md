...
- **Guarded auto-merge, off by default.** A PR only auto-merges when the target repo is on the `GOVERN_MERGE_REPOS` allowlist (which starts empty — you opt each repo in explicitly), its author is the governor's own worker identity, its branch matches the governor's naming (`ticket-<N>`, or the neutral `sl-<hex>` scheme on a `GOVERN_PUBLIC_REPOS` repo), its head is not from a fork, and CI is **green-or-no-checks**. Any factor missing → the PR stays open for a human. Frontends stay human-merged where a bad deploy is user-visible; backends with post-merge CI safety nets are safer to opt in.
...
- **`GOVERN_EXTERNALIZE_REPO`** + **`GOVERN_EXTERNALIZE_SUBREPO`** — turn on the externalization review lane. Each governor run **stages** every OPEN Low-severity ticket whose Where targets `GOVERN_EXTERNALIZE_SUBREPO` into a review queue (`queue/tickets-externalize-review.md`) and files ONE escalation questionnaire — it does **not** auto-file anything. You answer the questionnaire with a disposition: **approve-all** (file every staged ticket as a public "good first issue" GitHub Issue on `GOVERN_EXTERNALIZE_REPO`, then de-stage it), **decide-later** (leave them staged; re-nudged next run — exactly one questionnaire open at a time), or **move-back:&lt;ids&gt;** (return the listed tickets to `tickets.md` stamped `Externalize: never` so they never re-stage; the rest stay staged). Issues are created only on approval. Both empty (default) = lane off.
...
- **`GOVERN_PUBLIC_REPOS`** — space-separated SHORT names of repos that are **public**, so no internal ticket id leaks onto their PRs. On a public repo a worker names its branch with the neutral, deterministic `sl-<hex>` scheme instead of `ticket-<N>` and keeps the ticket id out of the PR title, body, and commit subjects; the three-factor auto-merge guard additionally accepts that neutral branch **only** for public repos (private repos keep the strict `ticket-<N>` pattern, unchanged). This knob is the deterministic override; when it's empty the governor auto-detects each target repo's visibility via `gh repo view` (cached once per run) and a lookup failure is treated as **private** (fail-safe). The existing PR title/body scrub remains the unconditional backstop.
