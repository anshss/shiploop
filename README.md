...
- **`GOVERN_EXTERNALIZE_REPO`** + **`GOVERN_EXTERNALIZE_SUBREPO`** — turn on the externalization REVIEW GATE. The governor never auto-publishes: each run MOVES every OPEN Low-severity ticket whose Where targets `GOVERN_EXTERNALIZE_SUBREPO` out of the live queue into `queue/tickets-externalize-review.md` and files ONE questionnaire escalation asking you to `approve-all` (file them all as public GitHub Issues on `GOVERN_EXTERNALIZE_REPO`), `move-back:<ids>` (restore the listed tickets and never re-stage them), or `decide-later` (keep them staged; ask again next run). A public issue is filed ONLY on `approve-all`. Seeds "good first issue" work for outside contributors — behind an explicit human approval. Both empty (default) = lane off.
