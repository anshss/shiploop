---
name: shiploop
description: Self-improving multi-agent harness for a meta-repo — N independent git repos as one workspace, with a ticket backlog ground down by one right-sized headless agent per ticket and guarded auto-merge on green CI. Load when working inside such a workspace (sub-folders each have their own .git; the root holds scripts/, queue/tickets.md, and governor/), or when judging whether the pattern fits a project. Carries the operating commands, worktree model, ticket queue, governor loop, hooks, and anti-patterns. To scaffold or upgrade a workspace, use /shiploop:setup.
---
