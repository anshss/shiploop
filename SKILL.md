...
- **Ticket selection:** no args works the whole eligible backlog; one number works that ticket only; several numbers (`run-loop.sh 152 153 154`) work exactly that ticket SET, sequentially, in severity order — each is its own explicit target, never overwritten by the next. Add `--parallel[=N]` to fan the set (or, with no explicit numbers, the top-N eligible backlog tickets) out across N concurrent single-ticket drivers instead of running them one at a time; the per-ticket claim lock + bookkeep lock keep that exactly-once safe.
