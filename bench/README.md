...
- Nothing in the corpus is instrumented yet. Four of the levers the bench attributes savings to are
...
|---|---|
| `lever-events.jsonl` per run, for four of the levers | nothing yet. The emitter now ships on by default at runtime, but has not been live for any run in the existing corpus |
