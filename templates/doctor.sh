...
command -v jq   >/dev/null && ok "jq"   || warn "jq missing — hard-required by every govern script (resolve-ticket.sh, pre-dispatch-check.sh, ... fail closed at startup); also used by the worktree registry"
...
if [ -f "$ROOT/CLAUDE.md" ]; then
  ...
  if [ "$_cb_size" -gt "$_cb_budget" ]; then
    fail "CLAUDE.md is $_cb_size chars, over the $_cb_budget budget (re-sent every turn): run '/shiploop:compress' to work through the compression candidates. Nothing is ever moved automatically."
    if [ -f "$ROOT/governor/claudemd-trim-proposals.md" ]; then
      ...
      warn "CLAUDE.md $_cb_size/$_cb_budget chars, $_cb_props compression candidate(s) pending in governor/claudemd-trim-proposals.md: run '/shiploop:compress'"
    fi
  else
