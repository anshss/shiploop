...
#   commit + CAS-push to origin/main under BK_LOCK. No Claude context involved. This is the
#   RESOLVE path that lands a ticket resolution. A split-off sibling, context-budgets.sh,
#   handles the CLAUDE.md/learnings.md context-ratchet budgets (formerly this script's
...
# a REAL live-test pass (validation.ranLiveTest=true + non-empty evidence). validation-record.sh
# itself never clobbers a pre-existing (hand-written, richer) summary: we capture whatever
# repo-relative path it printed (freshly written OR already-kept) so the pointer (step 6) is
# recorded either way, exactly as before.
vdoc=""; vdoc_rel=""
...
if [[ "$ranlive" == "true" && -n "$evidence" ]]; then
  ...
  declare -a vr_pr_args=()
  while IFS= read -r pr_disp; do
    [[ -n "$pr_disp" ]] || continue
    vr_pr_args+=(--pr "$pr_disp")
  done < <(printf '%s' "$report" | jq -r '
    ([ .pr ] + (.prs // [])) | map(select(. != null and ((.repo // "") != "")))
    | map("\(.repo)#\(.number)" + (if (.url // "") != "" then " \(.url)" else "" end)) | .[]' 2>/dev/null || true)
  # set -e safety: validation-record.sh calls govern::die on a bad argument and exits 0 printing
  # NOTHING when its kill switch (GOVERN_VALIDATION_RECORD=0) is set. Either would abort this whole
  # resolve mid-bookkeep -- BEFORE the tickets.md commit in step 4 -- if we let the failure or an
  # empty-value `[[ ]] &&` tail propagate (the hub's set -euo pipefail rule). A missing sink record
