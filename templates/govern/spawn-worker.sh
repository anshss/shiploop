...
# lookup/formatting itself lives ONCE, in govern::gotcha_block (lib/common.sh): the interactive
# lane has no launcher to inject this FOR it, so its gotchas-for-paths.sh wrapper calls the SAME
# function on the same candidate shape, and both lanes see identical output for identical paths.
# `**Paths:**` is opt-in per entry; an untagged rule is invisible to this mechanism, exactly as it
# is today. GOVERN_GOTCHA_INJECT=0 disables the whole mechanism for BOTH lanes (test/assert.sh
# forces it off for the suite; this ticket's own test opts back in, per the local idiom for a new
# dispatch-path knob).
if [[ "${GOVERN_GOTCHA_INJECT:-1}" != "0" && ${#REPOS[@]} -gt 0 ]]; then
  ...
  if [[ -n "$_gotcha_cand" ]]; then
    # shellcheck disable=SC2086
    _gotcha_block="$(govern::gotcha_block $_gotcha_cand)"
    [[ -n "$_gotcha_block" ]] && prompt="$prompt
