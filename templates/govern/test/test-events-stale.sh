...
# run_done, so without a staleness guard status.sh's "running ... up 30d" line never expires. A
# worker is an in-session subagent with no pid of its own, so its OWN liveness (items 5-7 below) is
# decided by the age of its spawn event against GOVERN_WORKER_STALE_S, not by polling a process.
#
# Contract:
...
#   5. statusline-segment.sh: a claimed-live worker whose OWN spawn event is older than
#      GOVERN_WORKER_STALE_S is treated as dead (segment stays silent, its own contract).
#   6. statusline-segment.sh: GOVERN_STATUSLINE_STALE_CHECK=0 restores the old behavior (reports the
#      stale claim as live, regardless of its age).
...
NOW_TS="$(date +%s)"
TS_OLD=$(( NOW_TS - (10 * 86400) ))   # 10 days ago: over the default 7-day window AND the default
                                       # 2h GOVERN_WORKER_STALE_S bound
TS_RECENT=$(( NOW_TS - 600 ))          # 10 minutes ago: well inside both windows
...
printf '{"ts":%s,"run_id":"r-old","type":"worker_spawned","agent_id":"aid-5","agent_type":"worker"}\n' "$((TS_OLD+10))" >> "$EV"
...
printf '{"ts":%s,"run_id":"r-new","type":"worker_spawned","agent_id":"aid-6","agent_type":"worker"}\n' "$((TS_RECENT+5))" >> "$EV"
...
printf '{"ts":%s,"run_id":"r-old","type":"worker_spawned","agent_id":"aid-77","agent_type":"worker"}\n' "$TS_OLD" > "$EV2"
out_seg="$(seg "$DEEP")"
assert_eq "$out_seg" "" "5. segment: a claimed-live worker whose spawn event is 10d old is treated as dead (silent)"

out_seg_off="$(seg "$DEEP" GOVERN_STATUSLINE_STALE_CHECK=0)"
assert_contains "$out_seg_off" "aid-77" "6. kill switch GOVERN_STATUSLINE_STALE_CHECK=0 reports the stale claim as live"

printf '{"ts":%s,"run_id":"r-new","type":"worker_spawned","agent_id":"aid-88","agent_type":"worker"}\n' "$TS_RECENT" > "$EV2"
out_seg_recent="$(seg "$DEEP")"
assert_contains "$out_seg_recent" "aid-88" "7. segment: a recent spawn is reported normally"
