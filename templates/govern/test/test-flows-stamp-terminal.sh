...
# validation/evidence/ (never .claude/context/validation/), and a stamp RACING a concurrent registry
...
# ── PASS → resolve semantics (correctness kind → Status=PASS), evidence promoted under validation/evidence/.
seed_flows
...
assert_eq "$(govern::flow_field deploy.gpu.vastai Evidence "$FLOWS")" "validation/evidence/deploy.gpu.vastai.md" "flows_stamp: Evidence field points at the tier-2 sink"
assert_eq "$([[ -f "$M/validation/evidence/deploy.gpu.vastai.md" ]] && echo yes || echo no)" "yes" "flows_stamp: evidence file written to validation/evidence/"
...
cat > "$T/A/validation/flows.md" <<EOF
## deploy.gpu.vastai
- **Kind:** correctness
- **Surface:** provision → snapshot → restore
- **Paths:** backend/**
...
assert_eq "$(govern::flow_field deploy.gpu.vastai Status "$T/A/validation/flows.md")" "PASS" "flows_stamp+race: local checkout carries our stamp"

V="$T/verify"; git clone -q "$T/origin.git" "$V"
assert_contains "$(cat "$V/validation/flows.md")" "**Status:** PASS" "flows_stamp+race: our stamp reached origin/main despite the race"
assert_eq "$([[ -f "$V/bookkeep.txt" ]] && echo yes || echo no)" "yes" "flows_stamp+race: concurrent bookkeep commit preserved on origin/main"
assert_eq "$([[ -f "$V/validation/evidence/deploy.gpu.vastai.md" ]] && echo yes || echo no)" "yes" "flows_stamp+race: evidence file committed to origin/main alongside the stamp"
