...
vfile="$T/.claude/shiploop/validation/ticket-40-validation-does-cross-provider-restore-round-trip.md"
...
assert_eq "$(cd "$T" && git status --porcelain .claude/shiploop/validation/ticket-40-validation-does-cross-provider-restore-round-trip.md | wc -l | tr -d ' ')" "0" "promote: summary committed, working tree clean"
...
assert_contains "$ptr" ".claude/shiploop/validation/ticket-40-validation-does-cross-provider-restore-round-trip.md" "pointer: history entry records the validationDoc path"
...
assert_eq "$(ls "$T/.claude/shiploop/validation"/ticket-41-* 2>/dev/null | wc -l | tr -d ' ')" "0" "no-promote: ordinary ticket writes no validation summary"
...
mk_tickets; : > "$HIST"
hand="$T/.claude/shiploop/validation/ticket-40-validation-does-cross-provider-restore-round-trip.md"
...
L="$(mktemp -d)"; mkdir -p "$L/.claude/shiploop/validation"
cat > "$L/CLAUDE.md" <<'EOF'
- Evidence: `.claude/shiploop/validation/ticket-11-x.md`
- Glob placeholder must NOT match: `.claude/shiploop/validation/ticket-<N>-*.md`
EOF
echo ok > "$L/.claude/shiploop/validation/ticket-11-x.md"
bash "$VLINT" "$L" >/dev/null 2>&1 && rc=0 || rc=1
assert_eq "$rc" "0" "lint: all refs resolve (and glob placeholder ignored) → exit 0"
# now add a dangling ref (founder-os context tree is a citation source even though the sink moved)
mkdir -p "$L/.claude/context"
cat > "$L/.claude/context/features.md" <<'EOF'
- Validation evidence: `.claude/shiploop/validation/ticket-99-gone.md`
EOF
...
assert_contains "$(cat "$BK")" ".claude/shiploop/validation" "bookkeep promotes into the committed validation sink"
