...
# ── 1. fresh write ──────────────────────────────────────────────────────────────────────────────
out="$(bash "$VR" --ticket 40 --title "VALIDATION: does cross-provider restore round-trip" \
...
# ── 3. never clobbers ───────────────────────────────────────────────────────────────────────────
out2="$(bash "$VR" --ticket 40 --title "VALIDATION: does cross-provider restore round-trip" \
...
out3="$(bash "$VR" --ticket 41 --title "!!!" --evidence "x")"
...
printf 'multi-line\nevidence body, PASS\n' > "$efile"
out4="$(bash "$VR" --ticket 42 --title "SPIKE: does caching help" --evidence-file "$efile")"
...
out5="$(bash "$VR" --ticket 43 --title "VALIDATION: dry preview" --evidence "x" --print-path-only)"
...
# ── 4. kill switch ───────────────────────────────────────────────────────────────────────────────
out6="$(GOVERN_VALIDATION_RECORD=0 bash "$VR" --ticket 44 --title "VALIDATION: killed" --evidence "x")"
rc6=$?
