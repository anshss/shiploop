#!/usr/bin/env bash
# Drift guard for the interactive worker (`.claude/agents/worker.md`).
#
# The worker doctrine lives in ONE canonical file, governor/worker-prompt.md, and
# `.claude/agents/worker.md` INCLUDES IT BY REFERENCE, telling the spawned worker to read
# that file first, rather than carrying a second copy. A hand-forked copy would silently
# re-create the drift this design exists to kill, so the assertions below fail on any sign
# of one.
#
# Also pins parts of the frontmatter to the shared govern defaults they must not drift from:
# model to GOVERN_WORKER_MODEL's floor (templates/lib/workspace.sh) and permissionMode to
# the same default every other headless `claude` dispatch in govern uses
# (templates/govern/sync-port.sh), so bumping one without the other is red.
#
...
# ── 2. Model floor matches the shared govern default ───────────────────────
# templates/lib/workspace.sh:  GOVERN_WORKER_MODEL="${GOVERN_WORKER_MODEL:-sonnet}"
floor_model="$(sed -n 's/.*GOVERN_WORKER_MODEL:-\([a-z0-9.-]*\)}.*/\1/p' "$WORKSPACE_LIB" | head -1)"
assert_eq "$([ -n "$floor_model" ] && echo yes || echo no)" "yes" \
  "2. read the GOVERN_WORKER_MODEL floor out of lib/workspace.sh"
assert_eq "$(sed -n 's/^model: *//p' <<<"$fm")" "$floor_model" \
  "2b. worker.md model matches the GOVERN_WORKER_MODEL floor ($floor_model)"
...
# ── 6. Capability posture matches govern's other headless dispatches ───────
# permissionMode: worker.md declares the same default govern's other headless `claude`
# dispatches hardcode (sync-port.sh: --permission-mode "${GOVERN_PERMISSION_MODE:-bypassPermissions}").
# The interactive lane has no per-run CLI invocation of its own to attach a flag to, so this has
# to live in the frontmatter.
sync_port_body="$(cat "$SYNC_PORT")"
shared_permission="$(sed -n 's/.*GOVERN_PERMISSION_MODE:-\([a-zA-Z]*\)}.*/\1/p' "$SYNC_PORT" | head -1)"
assert_eq "$([ -n "$shared_permission" ] && echo yes || echo no)" "yes" \
  "6b. read the shared permission-mode default out of sync-port.sh"
...
# settings.json level (scaffold.sh writes it project-wide; every subagent loads project settings). A
...
# ── 7. Hazard handoff reaches the worker ────────────────
# Nothing injects a "Recorded gotchas" section into a worker's prompt for it, so worker.md must
# tell it to run the lookup itself, on the paths it is about to touch, as an explicit step -- not
# leave it as a passive "go read the file" pointer.
...
assert_eq "$([ -f "$GOTCHA_SCRIPT" ] && echo yes || echo no)" "yes" \
  "7b. the script worker.md points at actually exists in templates/govern"

# ── 8. the consult goes UP to the advisor, and nothing ever spawns one ────
# Nothing ever spawns an advisor: the worker asks the session that wrote its proposal. Cases 8e-8g
# are REGRESSION tests on the ABSENCE of the old spawn-a-fresh-child instruction: re-introducing it
# in either doctrine file turns this red.
assert_contains "$body" "advisor-consult.sh claim" \
  "8b. worker.md uses the same script-owned budget/ledger"
...
assert_not_contains "$PROMPT_BODY" "spawn exactly ONE" \
  "8f. worker-prompt.md carries no spawn-a-fresh-child instruction either"
...
assert_contains "$PROMPT_BODY" "escalation" \
  "8h. worker-prompt.md's answer to an unresolvable fork is an honest escalation"

# ── 9. the ticket's proposal is implemented, in worker-prompt.md ───────────
assert_contains "$PROMPT_BODY" "Implement the ticket's" \
  "9. worker-prompt.md states the implement-the-proposal doctrine (worker.md includes this file by reference)"
