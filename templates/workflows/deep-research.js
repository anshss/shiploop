...
// script at ~/.claude/projects/.../workflows/scripts/deep-research-wf_e4b08f24-12d.js). The only
// change is the model-plan layer + cheap defaults; the pipeline, schemas, and verifier logic
// are unchanged so callers see the same output shape.
...
const searchResults = await pipeline(
  ...
  angle => agent(SEARCH_PROMPT(angle), stageOpts('search', {
  ...
  searchResult => {
    ...
    return parallel(
      novel.map(source => () => {
        ...
        return agent(FETCH_PROMPT(source, searchResult.angle), stageOpts('fetch', {
          label: "fetch:" + host,
          phase: "Fetch",
  ...
  rankedClaims.map(claim => () =>
    parallel(
      Array.from({ length: VOTES_PER_CLAIM }, (_, v) => () =>
        agent(VERIFY_PROMPT(claim, v), stageOpts('verify', {
